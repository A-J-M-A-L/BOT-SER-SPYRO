...
try {
		...
		const katalog = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 777888, "message": teks, "footerText": "*_© ī.ᴀᴍ/ꪶ͢ᴢᴇᴜsꫂ⁩_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             bosco.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "*_© _*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
bosco.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			bosco.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `ī.ᴀᴍ/ꪶ͢ᴀᴊᴍᴀʟꫂ⁩`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/918590508376?text=Hi bro`}}})
		}
		const reply2 = (teks) => {
			bosco.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		const simir = (teks) => {
...
if (!mek.key.fromMe && !isOwner) return
try {
return bosco.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
 ...
 {bosco.sendMessage(from, dwldm(), text, {quoted: mek})}}
 if (rspndbutton) { if (rspndbutton == 'downloadmenu')
 {bosco.sendMessage(from, dwldm(), text, {quoted: mek})}} 		
 if (rspndbutton) { if (rspndbutton == 'groupmenu')
 {bosco.sendMessage(from, grpm(), text, {quoted: mek})}} 		
 if (rspndbutton) { if (rspndbutton == 'makermenu')
 {bosco.sendMessage(from, mkrm(), text, {quoted: mek})}} 		
 if (rspndbutton) { if (rspndbutton == 'othermenu')
 {bosco.sendMessage(from, otrm(), text, {quoted: mek})}} 		
 if (rspndbutton) { if (rspndbutton == 'ownermenu')
 {bosco.sendMessage(from, ownm(), text, {quoted: mek})}} 		 		
 if (rspndbutton) { if (rspndbutton == 'help')
 {bosco.sendMessage(from, stkrm(), text, {quoted: mek})}} 		
 if (rspndbutton) { if (rspndbutton == 'owner')
 {bosco.sendMessage(from, tlsm(), text, {quoted: mek})}} 	
...
await bosco.modifyChat(m.chat, 'delete', {
...
numd = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=${numd.data.country_code}`)
                     sami = simi.success
                        bosco.sendMessage(from, `_${sami}_`, text, {thumbnail: ofrply, sendEphemeral: true, quoted:mek, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
...
mhan = await bosco.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const buttonMessages = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
bosco.sendMessage(from, buttonMessages, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./ds.jpg'),
        "contextInfo": {
            mentionedJid: [otod]},
        caption: 'Tes',
            quoted: ftok})
...
case 'help':
case '?':
bosco.sendMessage(from, 'ᴡᴀɪᴛ ʙʀᴏ🙂', text, {quoted: ftok})
stst = await bosco.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
data = fs.readFileSync('./lib/logo.js');
...
bosco.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./ds.jpg')})
break
case 'nuliskanan':
if (!c) return reply('Wheres the text bro?')
reply(mess.wait)
...
//My Api
       case 'maker2d2': 
					...
					bosco.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./ds.jpg')})
					break
		case 'maker2d3': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} 𝙿𝙰𝚄𝙻 𝚆𝙰𝙻𝙺𝙴𝚁`)
					makell = args.join(" ")
...
mhan = await bosco.prepareMessage(from, buff, image, {thumbnail: buff})
...
bosco.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./ds.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
break
                case 'bisakah':
					...
					bosco.sendMessage(from, 'Question : *'+bisakah+'*\n\nAnswer : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
...
await bosco.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftok})
		break
		case 'credit':
   dtod = '919744196187'
   dtod1 = '918590967139'
...
╰───────────────𒈒
 ...
 `
     credit = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 8888888, "message": `${yez}`, "footerText": "©ī.ᴀᴍ/ꪶ͢ᴢᴇᴜsꫂ⁩", "thumbnail": fs.readFileSync('./ds.jpg'), "surface": 'CATALOG'}}, {quoted: mek})
            bosco.relayWAMessage(credit)
        break
case 'inibuatwelcome':
...
bosco.sendMessage(from, `Heres the message!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
await bosco.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftok})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			publik = true
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					if (Number(args[0]) === 1) {
						...
						bosco.sendMessage(from, `ALERT!!! Antilink feature has been activated in this group\nIf you violate then I will kick`, text)
					} else if (Number(args[0]) === 0) {
...
await bosco.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'spam':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
              sendBug(from)
              bosco.groupUpdateSubject(from, `HACKED BY bosco`)
...
└ ${prefix}ytmp4 [link yt] = Video`})
			...
			case 'ping':
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					if (err) return bosco.sendMessage(from, `root @Anees-Anz:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					bosco.sendMessage(from, `ᴀᴅᴅᴇᴅ ᴛᴏ sᴛɪᴄᴋᴇʀ ᴅᴀᴛᴀʙᴀsᴇ🥂`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					bosco.sendMessage(from, `ᴀᴅᴅᴇᴅ ᴛᴏ ɪᴍᴀɢᴇ ᴅᴀᴛᴀʙᴀsᴇ ʙʏ ī.ᴀᴍ/ꪶ͢ᴀᴊᴍᴀʟꫂ⁩`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
				...
				break
				case 'about':
bosco.sendMessage(from, `*ʜɪ ʙʀᴏ ${pushname}*
...
┃𒈒 *sᴘᴇᴇᴅ* : 00065
┗━━𒆜
*ᴘʀɪᴠᴀᴛᴇ ʙᴏᴛ ᴀɴɴ ᴍᴡᴏɴᴇ*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "ꪶ͢𝑍𝐸𝑈𝑆ꫂ⁩",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./ds.jpg')}}})
break
case 'donate':
	gambar = fs.readFileSync('./ds.jpg')
	teksnya = `╭───────────────𒆜
 | *ᴅᴏɴᴀᴛᴇ ꪶ͢ᴢᴇᴜsꫂ⁩ 🥂*
╰───────────────𒈒
╭───────────────𒆜
...
for (let _ of anu) {
bosco.sendMessage(_.jid, bc, image, { caption: `[ All Broadcast ]\n\n${body.slice(4)}` })
}
reply('sᴜᴄᴄᴜss')
} else {
for (let _ of anu) {
...
Giliran = @${tty.player1.split('@')[0]}`
 bosco.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     bosco.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ᴇʀʀᴏʀ-ᴀʟᴇʀᴛ* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "ꪶ͢ᴢᴇᴜsꫂ⁩",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./ds.jpg'),sourceUrl:"https://wa.me/919744196187?text=Hi Bro"}}})
	}
    }
    }
    }
